...
path = r"C:\Users\naesboma\00_an\FEM_Q\Calculations\25_01_08_CrackEl\LS0"
# 1.2 Import
...
# 2.3.3 Principal directions
str1 = 'ip'
str2 = 'thinfa'
thinf = vect_for_txt_export(str1,str2)
thinf = np.concatenate((thinf,np.zeros((len(thinf[:,0]),3))),axis=1)

str2 = 'thsupa'
thsup = vect_for_txt_export(str1,str2)
thsup = np.concatenate((thsup,np.zeros((len(thsup[:,0]),3))),axis=1)

theta = np.concatenate((thinf,thsup[:,[3,4,5,6]]),axis=1)
np.savetxt(os.path.join(path, 'theta.txt'),theta)
