...
class stress():
    def __init__(self,sprev_klij,cm_klij,MAT,GEOM,k, l, i, j, cmcc_klij = 1,cmcs_klij = 1,cms_klij = 1,cmtn_klij = 1):
    ...
    def principal(self):
        ...
        if self.e1 > self.ect and self.e3 > self.ect:
            self.submodel = 3
        elif self.e1 < self.ect and self.e3 < self.ect:
            self.submodel = 1
        else:
            self.submodel = 2
        # self.submodel = 2
    ...
    def crack_kin(self,testmethod = False):
        ...
        def tct_scnr():
            ...
            if self.fcp >= 65:
                R_ai = min([0.85 * sqrt((7.2 / (self.fcp - 40) + 1) ** 2 - 1) + 0.34, 1])
            else:
                R_ai = 1
            ...
            if self.cmtn_klij == 1:
                ...
                if abs(self.dt) > 10 ** -10:
                    self.tctnr = R_ai * t0 * (1 - sqrt(2 * self.dn / Dmax)) * self.dt / self.dn * (
                                2.45 / t0 + 2.44 * (1 - 4 / t0) * abs(self.dt / self.dn) ** 3) / (
                                             1 + 2.44 * (1 - 4 / t0) * (self.dt / self.dn) ** 4)
                    self.scnr = R_ai * -0.62 * self.dt / (self.dn ** 2 + self.dt ** 2) ** (1 / 4) * self.tctnr
                else:
                    self.tctnr = 0
                    self.scnr = 0
            ...
            # 2 Interlock stresses according to Li and Maekawa
                ...
                tlim = Kd*3.83*self.fcp**(1/3)
                if abs(self.dt) > 10**-10:
                    self.tctnr = R_ai * self.dt / abs(self.dt) * tlim * self.dt ** 2 / (self.dn ** 2 + self.dt ** 2)
                    self.scnr = R_ai * -tlim*(pi/2-atan(abs(self.dn/self.dt))-abs(self.dn*self.dt/(self.dn**2+self.dt**2)))
                else:
                    self.tctnr = 0
                    self.scnr = 0
    ...
    def fcs(self):
        """ ---------------------- Calculation concrete strength with softening as a function of e1---------------------
            ...
            - fc(1):        Concrete strength considering softening according to CSFM (fib/SIA modified)
    ...
    def sigma_cart_31(self):
        ...
        if self.rhoy > 0:
            ...
            self.ssy = 0
        if self.k == 0 and self.i == 0 and self.j == 0 and self.l == 10:
            print('Activated sig_cart_31')
            print(f'rhox: {self.rhox}, rhoy: {self.rhoy}')
        ...
        if self.k == 0 and self.i == 0 and self.j == 0 and self.l == 10:
            print('Activated sig_cart_31')
            print('scx, scy', self.scx,  ' ', self.scy)
            print('ssx, ssy', self.ssx,  ' ', self.ssy)
    ...
    def sigma_cart_32(self):
        ...
        if self.cm_klij == 4:
            ...
            s1prev = mprev + rprev
            if self.th*self.thr < 0:
                self.cm_klij = 3
                self.principal()
            if s1prev < self.fct*.5:
                self.cm_klij = 3
                self.principal()
            elif self.e3 > -10**-4*0:
                self.cm_klij = 3
                self.principal()
            # elif abs(self.thr) > 80/90*pi/2 or abs(self.thr) < 10/90*pi/2:
            #     self.cm_klij = 3
            #     self.principal()
