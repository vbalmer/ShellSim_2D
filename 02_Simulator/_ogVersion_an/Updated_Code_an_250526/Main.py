...
# 0 Import
# -------------------------------------------------------------------------------------------------------------------- #
from Mesh_gmsh import MATK,gauss_order,numls,NODESG,it_type
...
def saveres(j,u,e,ex,ey,gxy,B,gauss_order,s,sh0,e0,MATK,fi,fe,f0,cDOF, relun, relthn):
    ...
    thz = u[5::6]
    sh = find_sh(s, 1) + sh0
    [ssx, ssy, spx, spy, e1, e3, th] = find_ss(s, MATK["cm"])
    if max(MATK["cm"]) < 1.5:
    ...
    POST = post(ux, uy, uz, r, thx, thy, thz, Nx, Ny, Nxy, Mx, My, Mxy, Qx, Qy, ex, ey, gxy, e3, e1, th, ssx, ssy, spx,
    ...
    path = os.path.join(os.getcwd(),'02_Simulator\\ogVersion_an\\Updated_Code_an_250526\\results')
