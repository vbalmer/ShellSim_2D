...
class stress():
    ...
    def crack_kin(self,testmethod = False):
        ...
        def tct_dowel():
            ...
            if abs(self.dt) < .02:
                act_dt = abs(self.dt) / .02
            else:
                act_dt = 1

            # 0.3 Crack Opening including numerical stability
            dy = (self.dn*cos(self.thr)+self.dt*sin(self.thr)) * act_dt
            ...
            # 1 Dowel Action according to Pejatovic and Muttoni
                ...
                # 1.3 Dowel force and resulting stresses
                Dd = beta**3*self.Esx*Is*dy
                Ddmax = (self.fsux-self.ss_bilin(self.ex, self.fsyx, self.Esx, self.Eshx))*self.dx**2*pi/4/sqrt(3)
                if Dd > Ddmax:
                    Dd = Ddmax
                tdow = abs(Dd/(sz*sy))
            ...
            # 2 Dowel Action according to Vintzeleou and Tassios 1986
                ...
                # effective concrete tensile strength depending on concrete stress state at the crack
                th_star = atan(-self.tcxy/(self.scx-self.fct))
                _, scy_star, _ = mohr_1(self.fct, self.sc3, th_star, True)
                fct_star = scy_star-self.scy

                # Flag
                # if self.fcp == 28.1 or self.fcp == 27.7:
                fct_star = self.fct

                # calculation of force and deformation
                Du2 = xip * (sz - self.dx) * self.dx * fct_star
                Ddmax = (self.fsux-self.ss_bilin(self.ex, self.fsyx, self.Esx, self.Eshx))*self.dx**2*pi/4/sqrt(3)
                if Du2 > Ddmax:
                    Du2 = Ddmax
                ...
                Du2lim = 0.5 * (sz - self.dx) * self.dx * fct_star
        ...
        if testmethod:
            ...
            if dotest == 'Agg':
                steps = 100
                dt_all = np.linspace(0.001, 6, steps)
                dn_all = [.5+2*i*0 for i in dt_all]
                ...
                steps = 100
                dt_all = np.linspace(0.001, 6, steps)
                dn_all = np.linspace(0.001, 6, steps)
                dy_all = np.zeros_like(dt_all)
                tau_all = np.zeros_like(dt_all)
                sigma_all = np.zeros_like(dt_all)
                self.sr = 150
                self.srx = self.sr
                ...
                for j in range(len(dt_all)):
                    ...
                    dy_all[j] = (self.dn*cos(self.thr)+self.dt*sin(self.thr))
    ...
    def spr(self, e, ec, rhop, srm, d, fpu, Ep, tb):
        ...
        if self.cms_klij == 0 or e <= ec + 1e-15:
            ...
            # 1 Seelhofer
            x1, spr, _ = self.ssr_seelhofer(e, ec, rhop, srm, d, fpu, Ep, Ep, tb, tb)
            # 2 TCM
            if x1 >= srm / 2:
                spr = e*Ep+tb*srm/d
            if spr > fpu:
    ...
    def ssr_seelhofer(self, e, ec, rho, srm, d, fsy, Es, Esh, tb0, tb1):
        ...
        #         ssr = ssr-Es*.3/1000
        #     elif x1 < srm/2:
        #         ssr = ssr-Es*.3/1000*((srm/2-x1)/(srm/4))
        return x1,ssr,tb0
    ...
    def sr0_vc(self):
        ...
        # Flag
        # if self.rhoy == 0 and self.rhox == 0.0088:
        #     lbd = 0.67*2
        # else:
        #     lbd = 0.67

        # Flag
        # if self.rhox == 4.942/100:
        #     lbd = 0.67*1.24
        # else:
        #     lbd = 0.67
        ...
        # 1.2 in y
        if rhoy + rhopy > 10**-9:
            self.sry0 = (2 * tb0 / dy * rhoy + 2 * tbp0 / dpy * rhopy) ** (-1) * fct * (
                        1 - rhoy - rhopy)
        else:
            self.sry0 = 10**3
    ...
    def sigma_cart_1(self):
        ...
        self.ssx = (self.ex+ecs)*self.Esx
        self.ssy = self.ey*self.Esy
        self.spx = (self.ex+ecs)*self.Epx
    ...
    def sigma_cart_31(self):
        ...
        # 3 CFRP Contribution
        if self.rhopx > 0:
            self.spx,_ = self.spr(self.ex, self.ecx, self.rhopx, 100, self.dpx, self.fpux, self.Epx, self.tbp0)
        else:
            self.spx = 0
        if self.rhopy > 0:
            self.spy,_ = self.spr(self.ey, self.ecy, self.rhopy, 100, self.dpy, self.fpuy, self.Epy, self.tbp0)
        else:
            self.spy = 0
    ...
    def out(self,e):
        ...
        -------------------------------------------------------------------------------------------------------------"""
        # 1 Manipulate Strains so they're not 0
        # if abs(e[0]) < 10**-7:
        #     e[0] = cplx(-10**-7,e[0].imag)
        # if abs(e[1]) < 10**-7:
        #     e[1] = cplx(-10**-7,e[1].imag)
        # if abs(e[2]) < 10**-8:
        #     e[2] = cplx(-10**-8,e[2].imag)
