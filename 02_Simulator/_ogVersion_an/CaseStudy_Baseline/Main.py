...
# 0 Import
# -------------------------------------------------------------------------------------------------------------------- #
from Mesh_gmsh_copy import MATK,gauss_order,NODESG,it_type
...
[e0, ex0, ey0, gxy0, e10, e30, th0] = find_e0(gauss_order)
[e0c, ex0c, ey0c, gxy0c, e10c, e30c, th0c] = find_e0(1)
...
[e,ex,ey,gxy,e1,e3,th] = find_e(e0,eh,gauss_order)

s = find_s(e,gauss_order)
...
if numit == 0:
    ...
    for i in range(numit):
        ...
        plot_convergence(i, e, rele,un,relun,thn,relthn,r)
...
Bc = find_b(1)
ehc = find_eh(Bc,u,1)
ec = find_e(e0c,ehc,1)[0]
sc = find_s(ec,1)
sh = find_sh(sc,1)+sh0
if max(MATK["cm"]) > 1:
    [ssx, ssy, spx, spy] = find_ss(s,MATK["cm"])
else:
    ssx = np.zeros_like(ex)
    ssy = np.zeros_like(ex)
    spx = np.zeros_like(ex)
    spy = np.zeros_like(ex)
[e, ex, ey, gxy, e1, e3, th] = find_e(np.zeros_like(e0), eh, gauss_order)
Nx = sh[:, :, :, 0]
Ny = sh[:, :, :, 1]
Nxy = sh[:, :, :, 2]
Mx = sh[:, :, :, 3]
...
# 4.0 Import
from Mesh_gmsh_copy import ELS, COORD,GEOMA,MASK,GEOMK,na,BC
ELEMENTS = ELS[0]
...
POST = post(ux,uy,uz,r,thx,thy,thz,Nx,Ny,Nxy,Mx,My,Mxy,Qx,Qy,ex,ey,gxy,e3,e1,th,ssx,ssy,spx,spy,rele,relun,relthn)
...
path = r"C:\Users\vbalmer\OneDrive\Dokumente\GitHub\research_vera\02_Computations\06_LinElData\02_Simulator\_ogVersion_an\Beispielrechnung\results"
