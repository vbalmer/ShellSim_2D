...
class stress():
    def __init__(self,sprev_klij,cm_klij,MAT,GEOM,cmcc_klij = 1,cmcs_klij = 1,cms_klij = 1,cmtn_klij = 1,cmd_klij = 2):
        """ --------------------------------- Initiate instance of stress class-----------------------------------------
            ...
            - cmd_klij:      Model for Dowel Action
                             0 = No dowel action
                             1 = Pejatovic and Muttoni (2024)
                             2 = Vintzeleou and Tassios (1986)
    ...
    def principal(self):
        ...
        if self.e1 > self.ect*0 and self.e3 > self.ect*0:
            self.submodel = 3
        elif self.e1 < self.ect*0 and self.e3 < self.ect*0:
            self.submodel = 1
        else:
            self.submodel = 2
        # self.submodel = 2
        ...
        if self.cm_klij == 4 and self.submodel == 2 and self.sprev_klij.iscracked == 1:
    ...
    #         ----------------------------------------    INPUT (self.): -------------------------------------------------
    #         - ex, ey, gxy:  In plane strains
    #         - s_prev: crack direction from previous iteration
    #         ------------------------------------------- OUTPUT (self.):-------------------------------------------------
    ...
    #                              1 / 2 * sqrt((self.scxk - self.scyk) ** 2 + 4 * self.tcxy ** 2), color='gray',
    #                              fill=False)
    ...
    #         ax[1].plot([self.scx,self.scx+self.dsxrk],[self.tcxy,self.tcxy],color='b')
    #         ax[1].plot([self.scy, self.scy + self.dsyrk], [-self.tcxy, -self.tcxy], color='b')
    ...
    #         ax[1].plot([self.scxk, self.scxk + rr * cos(self.thr)], [-self.tcxy, -self.tcxy + rr * sin(self.thr)],
    #                    color='gray', linewidth=0.5, linestyle='dashdot')
    #         ax[1].plot([self.scxk, self.scxk + rr * cos(self.thck)], [-self.tcxy, -self.tcxy + rr * sin(self.thck)],
    ...
    #         Dmax = 16
    #         if self.dn < 0.00001:
    #             self.dn = self.dn / self.dn * 0.00001
    #
    #         # 1 Interlock stresses according to Gambarova and Karakoc
    #         t0 = 0.35 / 0.35 * 0.3 * self.fcp
    #         if abs(self.dt) > 10 ** -10:
    #             self.tctnr = 1 * t0 * (1 - sqrt(2 * self.dn / Dmax)) * self.dt / self.dn * (
    #                         2.45 / t0 + 2.44 * (1 - 4 / t0) * abs(self.dt / self.dn) ** 3) / (
    #                                      1 + 2.44 * (1 - 4 / t0) * (self.dt / self.dn) ** 4)
    #             self.scnr = 1 * -0.62 * self.dt / (self.dn ** 2 + self.dt ** 2) ** (1 / 4) * self.tctnr
    #         else:
    #             self.tctnr = 0
    #             self.scnr = 0
    #
    ...
    #     self.dsxrk = (self.ssx - self.ssxk) * self.rhox / (1 - self.rhox) + (self.spx - self.spxk) * self.rhopx / (
    #             1 - self.rhopx)
    #     self.dsyrk = (self.ssy - self.ssyk) * self.rhoy / (1 - self.rhoy) + (self.spy - self.spyk) * self.rhopy / (
    #             1 - self.rhopy)
    #     self.dsxrk = self.dsxrk*1
    #     self.dsyrk = self.dsyrk*1
    #
    #
    #     # 2.2 Mohr's Circle of characteristic stress increments
    #     self.dsnrk,self.dstrk,self.dttnrk,self.ds1rk,self.ds3rk = mohr_x(self.dsxrk,self.dsyrk,0,True)
    #
    #     # 2.3 Backcalculate stress increments in x and y for control
    #     self.dsxrk_contr, self.dsyrk_contr, self.dtxyrk_contr, self.ds1rk_contr, self.ds3rk_contr = mohr_n(self.dsnrk, self.dstrk, self.dttnrk, True)
    #
    #     # 2.4 First estimates of strain increments
    #     self.denrk = self.dsnrk / self.Ec
    #     self.detrk = self.dstrk / self.Ec
    ...
    #         self.scxk = self.scx + self.dsxrk
    #         self.scyk = self.scy + self.dsyrk
    #         self.tcxyk = self.tcxy
    #         self.scnk, self.sctk, self.tctnk, self.sc1k, self.sc3k = mohr_x(self.scxk, self.scyk, self.tcxyk, True)
    ...
    #             self.scxk, self.scyk, _ = mohr_1(self.sc1k, self.sc3k, self.thck, True)
    #
    #             # 12 Mohr's Circle of Concrete stresses at the crack
    #             self.scx = self.scxk - self.dsxrk
    #             self.scy = self.scyk - self.dsyrk
    #             _, _, _, self.sc1, self.sc3 = mohr_x(self.scx, self.scy, self.tcxy, True)
    #
    ...
    #     # print(itcount)
    #
    #     # 15 Check concrete stresses at crack element centre and determine if a new crack is forming, adjust submodel
    #     #    accordingly
    #     ect_temp = self.fct/self.Ec
    #     # self.ec1max = self.ec1 + (self.ec1k - self.ec1) * 2
    #     self.ec1max = self.sc1/self.Ec
    #     # if self.ec1max > ect_temp:
    #     #     self.cm_klij = 3
    #     #     self.iscracked = 0
    #     #     self.submodel = 3
    #     if self.ern <= 0:
    ...
    #     if doplot and self.ex.imag > 0 and self.cm_klij == 4 and self.submodel == 2 and self.gxy > 0.008:
    #         plot_mohr()
    ...
    def crack_kin(self,testmethod = False):
        ...
        def tct_scnr():
            ...
            # if self.s1prev < self.fct * ff_fct * 2:
            #     acti = (-ff_fct*self.fct+self.s1prev)/(ff_fct*self.fct)
        ...
        def tct_dowel():
            """ --------------------------------Dowel Action according to specified model ------------------------------
                       ---------------------------------------------    INPUT: -----------------------------------------
                       - (self.) cmd_klij
                             0 = No dowel action
                             1 = Pejatovic and Muttoni (2024)
                             2 = Vintzeleou and Tassios (1986)
                       ------------------------------------------- OUTPUT (self.):--------------------------------------
                       - Dowel stresses in n and nt
                   --------------------------------------------------------------------------------------------------"""
            # 0.0: Calculations equal for all cases
            # 0.1 Assumption for spacings
            sy = self.spacx
            ...
            # 0.2 Activation factor for inclusion of dn in dowel action (numerical stability)
            if abs(self.dt) < .2:
                act_dt = abs(self.dt) / .2
            else:
                act_dt = 1

            # 0.3 Crack Opening including numerical stability
            dy = (self.gxy - self.gcxyk) * self.sr * act_dt

            # 0: No Dowel Action
            if self.cmd_klij == 0:
                tdow = 0

            # 1 Dowel Action according to Pejatovic and Muttoni
            elif self.cmd_klij == 1:
                # 1.1 Factors eta for Eq. 17
                eta_th = (abs(self.thr)/(pi/2))**(3/5)
                eta_d = min(1.5/(1+25*abs(dy)/self.dx),1)
                eta_c = 1-0.45/(1+(20/self.dx)**2)
                eta_cast = 0.45
                eta_fc = (self.fcp/30)**(2/5)
                eta_bond = (1/(1+self.ex*self.srx/0.2))**(1/6)
                eta_cyc = 1
                # print(eta_th,eta_d,eta_c,eta_cast,eta_fc,eta_bond,eta_cyc)

                # 1.2 Bearing stiffness
                kc = 0.2*self.Ec/self.dx*eta_th*eta_d*eta_c*eta_cast*eta_fc*eta_bond*eta_cyc
                beta = 2/self.dx*(kc*self.dx/(pi*self.Esx))**(1/4)
                Is = pi*self.dx**4/64

                # 1.3 Dowel force and resulting stressse
                Dd = beta**3*self.Esx*Is*dy
                tdow = abs(Dd/(sz*sy))

                # 1.4 Resulting normal stresses in the bar
                ss_flex = 0.37*sqrt(kc*self.dx*self.Esx)*dy/self.dx
                ss_super = ss_flex + self.ssx
                if ss_super > self.fsyx:
                    tdow = tdow*(self.fsyx-self.ssx)/ss_flex
                    if tdow < 0:
                        tdow = 0


            # 2 Dowel Action according to Vintzeleou and Tassios 1986
            elif self.cmd_klij == 2:
                # 2.0 Excentricity of dowel action: distance to concrete face
                ev = self.dx * ff_ev

                # 2.1 Failure mode I: Concrete crushing beneath bar
                fsyred = self.fsyx - self.ex * self.Esx
                if fsyred < 0:
                    fsyred = 0

                # 2.1.1 Components of Mitternachtsformel for Eq. (6) in Vintzeleou 1986
                av = 1
                bv = 10*self.fcp*ev*self.dx
                cv = -1.7*self.dx**4*self.fcp*fsyred

                # 2.1.2 Crushing force Du1 and pertaining deformation du1
                Du1 = (-bv+sqrt(bv**2-4*av*cv))/2
                du1 = 0.05 * self.dx
                # Du10 = 1.3 * self.dx ** 2 * sqrt(self.fcp * fsyred) # Case no excentricity

                # 2.2 Failure mode II: Concrete splitting
                # 2.2.1 xi_prime depending on excentricity
                if ev <= self.dx:
                    xip = 2-0.8*ev/self.dx
                elif ev <= self.dx*2:
                    xip = 1.2-0.4*(ev-self.dx)/self.dx
                elif ev <= self.dx * 3:
                    xip = 0.8 - 0.15 * (ev - 2*self.dx) / self.dx

                # 2.2.2 Splitting force Du2 and pertaining deformation du2
                Du2 = xip * (sz - self.dx) * self.dx * self.fct
                du2 = 4 * Du2 * (ev*5/(8*self.dx)+1) / self.Ec

                # 2.2.3 Splitting force and pertaining deformation at limit of e = 5d
                Du2lim = 0.5 * (sz - self.dx) * self.dx * self.fct
                du2lim = 4 * Du2 * (5*self.dx * 5 / (8 * self.dx) + 1) / self.Ec

                # 2.3 Resulting maximum stress
                if Du1 < Du2:
                    tdow_max = abs(Du1 / (sy * sz))
                    dmax = du1
                else:
                    tdow_max = abs(Du2 / (sy * sz))
                    dmax = du2

                # 2.4 limit stresses
                tdow_lim = abs(Du2lim / (sy * sz))
                dlim = du2lim

                # 2.4 Stress in dependence of crack opening in y-direction
                # 2.4.1 Dowel action depending on maximum stress and crack displacement: linear increase from 0 to dmax,
                #       linear decrease to 4dmax, after that 0
                if abs(dy) < dmax:
                    tdow = dy / dmax * tdow_max
                elif abs(dy) < dlim:
                    tdow = tdow_max - (tdow_max-tdow_lim)*(abs(dy)-dmax)/(dlim-dmax)
                else:
                    tdow = tdow_lim
                # elif abs(dy) < 4*dmax:
                #     tdow = (4*dmax-dy) / (3*dmax) * tdow_max
                # else:
                #     tdow = dy/abs(dy) * tdow_max*0

            # 4 Same for all calculations: Shear and normal stress in crack due to dowel action. sign of normal stress depends on in which
            #     direction the crack slip opens. Positive theta and positive dt --> tension in crack. opposite
            #     --> compression.
            self.tdtnr = tdow * abs(sin(self.thr)) * sin(self.thr)
            self.sdnr = tdow * abs(sin(self.thr)) * cos(self.thr)

            # Alternative calculation of sdnr and dtnr ia mohr's circles
            # sdnr, sdtr, tdtnr, _, _ = mohr_x(0, tdow*tan(self.thr), 0 , True)
            # print('sigma: ',self.sdnr,sdnr)
            # print('tau: ', self.tdtnr,tdtnr)


            self.tctnr += self.tdtnr
            self.scnr += self.sdnr
        # --------------------------------------------------------------------------------------------------------------
        "-"
        ...
        if testmethod:
            ...
            if dotest == 'Agg':
                ...
                steps = 100
                gxy_all = np.linspace(1e-6, 2e-2, steps)
                dy_all = np.zeros_like(gxy_all)
                tau_all = np.zeros_like(gxy_all)
                sigma_all = np.zeros_like(gxy_all)
                self.sr = 150
                self.srx = self.sr
                self.dt = 0.5
                self.dn = 0.5
                ...
                self.ssx = 100
                self.ssxk = 100
                self.ex = self.ssx/200000
                self.gcxyk = 0
                self.tctnr = 0
                self.scnr = 0
                for j in range(len(gxy_all)):
                    self.gxy = gxy_all[j]

                    tct_dowel()
                    tau_all[j] = self.tdtnr.real
                    sigma_all[j] = self.sdnr.real
                    dy_all[j] = self.gxy*self.sr
                return dy_all, dy_all, tau_all.real, sigma_all.real
        ...
        # 16 Plot Mohr's circle if option is checked
        if doplot and self.ex.imag > 0 and self.cm_klij == 4 and self.submodel == 2 and self.gxy > 0.002:
            plot_mohr()
    ...
    def ssr(self, e, ec, srm, rho, rhop, d, fsy, fsu, Es, Esh, tb0, tb1, Ec):
        ...
        if self.cms_klij == 0 or e <= ec + 1e-15:
            ...
            ssr = self.ssr_tcm_ro(e, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec)
    ...
    def spr(self, e, ec, rhop, srm, d, fpu, Ep, tb):
        ...
        if self.cms_klij == 0 or e <= ec + 1e-15:
            ...
            spr = self.ssr_tcm_custom(e, srm, self.rhopx, d, self.tbp0, self.tbp1, Ep)
        else:
            # 1 Seelhofer
            x1, spr = self.ssr_seelhofer(e, ec, rhop, srm, d, fpu, Ep, Ep, tb, tb)
            # 2 TCM
            if x1 >= srm / 2:
                spr = e*Ep+tb*srm/d
            if spr > fpu:
                spr = fpu + (e-fpu/Ep)*ff
                # print('CFRP failure?')
                # spr = fpu + (e-fpu/Ep)*2357
                # print('Achtung!! funktion spr für Stahl temporär')

        spk = spr-4*tb/d*srm/2
        if spk < Ep*ec:
            spk = Ep*ec
    ...
    def ssr_seelhofer(self, e, ec, rho, srm, d, fsy, Es, Esh, tb0, tb1):
        ...
        thresh = 10
        while x1 < thresh:
            x1 = sqrt(Es) * sqrt(-d * srm * (ec - e)) / (2 * sqrt(tb0))
            ssr = ec * Es + x1 * 4 * tb0 / d
            if x1 < thresh:
                tb0 = tb0 * (x1 / thresh) ** 2 * 0.999

        # Version 2.2
        # Ec = self.Ec
        # a = 2*tb0/(Es*d)
        # b = srm*2*tb0/(Ec*d)*rho/(1-rho)
        # c = srm/2*(ec-e)
        # x1 = -b+sqrt(b**2-4*a*c)/(2*a)
        # ssr = Es*(ec+x1*4*tb0/d*rho/(1-rho)/Ec)+x1*4*tb0/d

        # 3 Elastic - Plastic crack element
        if x1 < srm / 2 and ssr > fsy:
            print('Seelhofer elastic-plastic,srm=')
            x1 = (fsy - ec * Es) * d / (4 * tb0)
            x2 = -(d * (-sqrt((16 * tb1 * (-Es * ec * srm + Es * ec * x1 - fsy * x1 + Es * e * srm)) / d + (
                        2 * Es * ec - 2 * fsy) ** 2) - 2 * Es * ec + 2 * fsy)) / (8 * tb1)
            ssr = fsy + x2 * 4 * tb1 / d
            print(srm)
            print('ssr= ')
            print(ssr)
    ...
    def ssr_tcm_bilin(self, e, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec, flag = 0):
        """ ------------------------------------ Calculate Steel Stress with the TCM -----------------------------------
            ...
            ---------------------------------------------------------------------------------------------------------"""
        # 1 Seelhofer
        x1, ssr = self.ssr_seelhofer(e, ec, rho, srm, d, fsy, Es, Esh, tb0, tb1)
        ...
        # 3 If close to yielding: smooth
        # esy = fsy/Es
        # elim1 = esy * 9 / 10
        # elim2 = 1.33 * esy
        # if elim1 < e < elim2 and flag == 0:
        #     ssr1_1 = self.ssr_tcm_bilin(elim1, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec, 1)
        #
        # if elim1 < e < elim2 and flag == 0:
        #     ssr1_2 = self.ssr_tcm_bilin(elim1 - 0.0001, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec, 1)
        #
        # if elim1 < e < elim2 and flag == 0:
        #     ssr2_1 = self.ssr_tcm_bilin(elim2, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec, 1)
        #
        # if elim1 < e < elim2 and flag == 0:
        #     ssr2_2 = self.ssr_tcm_bilin(elim2 + 0.0001, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec, 1)
        #
        # if elim1 < e < elim2 and flag == 0:
        #     E1 = (ssr1_1 - ssr1_2) / 0.0001
        #     E2 = (ssr2_2 - ssr2_1) / 0.0001
        #
        #     x1 = elim1
        #     x2 = elim2
        #     y1 = ssr1_1
        ...
        #     y4 = E2
        #
        #     a = (x1 ** 2 * (-y3) * y4 + 2 * x1 * x2 * y3 * y4 - x2 ** 2 * y3 * y4 + y1 ** 2 - 2 * y1 * y2 + y2 ** 2) / (
        #                 (x1 - x2) * (
        #                     x1 ** 2 * y3 - x1 * x2 * y3 + x1 * x2 * y4 - x1 * y1 + x1 * y2 - x2 ** 2 * y4 - x2 * y1 + x2 * y2))
        #     b = (
        #                     x1 ** 3 * y3 * y4 - x1 ** 2 * x2 * y3 * y4 - x1 ** 2 * y1 * y4 - x1 ** 2 * y2 * y3 - x1 * x2 ** 2 * y3 * y4 + 2 * x1 * x2 * y1 * y4 + 2 * x1 * x2 * y2 * y3 + 2 * x1 * y1 * y2 - 2 * x1 * y2 ** 2 + x2 ** 3 * y3 * y4 - x2 ** 2 * y1 * y4 - x2 ** 2 * y2 * y3 - 2 * x2 * y1 ** 2 + 2 * x2 * y1 * y2) / (
        #                     (x1 - x2) * (
        #                         x1 ** 2 * y3 - x1 * x2 * y3 + x1 * x2 * y4 - x1 * y1 + x1 * y2 - x2 ** 2 * y4 - x2 * y1 + x2 * y2))
        #     c = (
        #                     -x1 ** 3 * x2 * y3 * y4 + x1 ** 3 * y2 * y3 + 2 * x1 ** 2 * x2 ** 2 * y3 * y4 + x1 ** 2 * x2 * y1 * y4 - 2 * x1 ** 2 * x2 * y2 * y3 - x1 ** 2 * y1 * y2 + x1 ** 2 * y2 ** 2 - x1 * x2 ** 3 * y3 * y4 - 2 * x1 * x2 ** 2 * y1 * y4 + x1 * x2 ** 2 * y2 * y3 + x2 ** 3 * y1 * y4 + x2 ** 2 * y1 ** 2 - x2 ** 2 * y1 * y2) / (
        #                     (x1 - x2) * (
        #                         x1 ** 2 * y3 - x1 * x2 * y3 + x1 * x2 * y4 - x1 * y1 + x1 * y2 - x2 ** 2 * y4 - x2 * y1 + x2 * y2))
        #     d = (x1 * y3 + x1 * y4 - x2 * y3 - x2 * y4 - 2 * y1 + 2 * y2) / (x1 ** 2 * (
        #         -y3) + x1 * x2 * y3 - x1 * x2 * y4 + x1 * y1 - x1 * y2 + x2 ** 2 * y4 + x2 * y1 - x2 * y2)
        #     ssr = (a * e ** 2 + b * e + c) / (1 + d * e)
        return ssr
    ...
    def ssr_tcm_ro(self, e, ec, srm, rho, d, fsy, fsu, Es, Esh, tb0, tb1, Ec):
        ...
        # 0 Define neccessary parameters
        esy2 = 0.002
        esy1 = 0.0001
        fs2 = fsy
        if d == 8:
        ...
        if x1 >= srm / 2:
            ...
            while abs(kit-1) > 0.001:
                if ssr <= fsy:
                    eres = ssr/Es-tb0*srm/(Es*d)+d/(2*tb0*srm)*esy2/((a+1)*fs2**a)*(ssr**(a+1)-(ssr-2*tb0*srm/d)**(a+1))
                elif ssr <= fsy+2*tb1*srm/d:
                    eres = d/(4*Es*tb1*srm)*((ssr-fsy)**2*(1-tb0/tb1)+2*Es*esy2/((a+1)*fs2**a)*(ssr**(a+1)-
                           fsy**(a+1)*(1-tb1/tb0)-tb1/tb0*(fsy+tb0/tb1*(ssr-fsy)-2*tb0*srm/d)**(a+1)))+\
                           tb0/tb1*(ssr/Es-fsy/Es*(1-tb1/tb0))-tb0*srm/(Es*d)
                else:
                    eres = ssr/Es-tb1*srm/(Es*d)+d/(2*tb1*srm)*esy2/((a+1)*fs2**a)*(ssr**(a+1)-(ssr-2*tb1*srm/d)**(a+1))
                kit = e/eres
                if e < fsy/Es:
                    ssr = ssr + (e-eres)*Es/4
                else:
    ...
    def ssr_pom(self, e, d, fsy, fsu, Es, Esh, tb0, tb1):
        """ ------------------------------------ Calculate Steel Stress with the POM -----------------------------------
                ------------------------------------------ INPUT (self.): ----------------------------------------------
                - e: normal strain
                - srm: Crack spacing: srm = lambda * sr0
                - rho: Reinforcement content
                ...
                - Ec: Concrete E-modulus
                --------------------------------------------- OUTPUT:---------------------------------------------------
                - ssr: steel stress at the crack according to POM
            ---------------------------------------------------------------------------------------------------------"""
        # 1 Distance of zero slip points for fsu
        lavg = 2*d*(fsy/(4*tb0)+(fsu-fsy)/(4*tb1))

        # 2 Calculation of eps_sr for assumption of linear steel

        # 2.1 Steel strain and stress at crack
        esr = sqrt(e * 4 * tb0 * lavg / (d * Es))
        ssr = Es * esr

        # 2.2 Control value according to eq. 3.6 CSFM Verification book
        em_control = ssr ** 2 * tb1 / tb0 / (2 * Es * (fsu + fsy * (tb1 / tb0 - 1)))

        # 3 Calculation of eps_sr for assumption of yielding steel
        if ssr > fsy:

            # 3.1 Steel stress at crack
            esy = fsy / Es

            # Constants for solving
            A1 = e * (fsu + fsy * (tb1 / tb0 - 1))
            A2 = esy * fsy * (tb1 / (2 * tb0) - 1) + fsy ** 2 / (2 * Esh)

            # Coefficients of Mitternachtsformel
            a = 1 / (2 * Esh)
            b = esy - fsy / Esh
            c = A2 - A1

            # Solution: steel stress at crack
            ssr = (-b + sqrt(b ** 2 - 4 * a * c)) / (2 * a)

            # 3.2 Control value according to eq. 3.6 CSFM Verification book

            em_control = (esy * (ssr + fsy * (tb1 / (2 * tb0) - 1)) + (ssr - fsy) ** 2 / (2 * Esh)) / (
                        fsu + fsy * (tb1 / tb0 - 1))

        # 4 Control
        if abs(e-em_control)/e > 0.001:
            print('Attention: Error in POM calculation')

        # 5 If stress > ultimate stress, assign ultimate stress
        if ssr > fsu:
            ssr = fsu
        return ssr
    ...
    def sr0_vc(self):
        ...
        -------------------------------------------------------------------------------------------------------------"""
        # 0 Determine procedure based on applied constitutive model
        th = self.thr
        ...
        # 0.2 If fixed cracks (cm_klij = 4): manipulate crack spacings to values of previous iteration
        # if self.cm_klij == 4 and self.sprev_klij.iscracked == 1:
        #     self.srx0 = self.sprev_klij.srx0.real
        #     self.sry0 = self.sprev_klij.sry0.real
        #     self.srx = self.sprev_klij.srx.real
        #     self.sry = self.sprev_klij.sry.real
        #     self.sr = self.sprev_klij.sr.real

            # self.sr1 = self.sr / cos(self.thr - self.th)
            # self.d = self.sr1 * self.e1
            # self.dn_contr = self.d * cos(self.thr - self.th)
            # self.dt_contr = self.d * sin(self.thr - self.th)
            # return
        ...
        srxmax = self.dx/(2*self.tb0)*self.fsyx + self.dx/(2*self.tb1)*(self.fsux-self.fsyx)
        # srxmax = 150
        # srxmax = srxmax*10
        if self.srx0 > srxmax:
            self.srx0 = srxmax
        ...
        srymax = self.dy/(2*self.tb0)*self.fsyy + self.dy/(2*self.tb1)*(self.fsuy-self.fsyy)
        # srymax = 150
        # srymax = srymax * 10
        if self.sry0 > srymax:
            self.sry0 = srymax
        ...
        if abs(sin(th)) > 0:
            ...
            # print('Achtung: Sonderfall theta, Stresses_mixreinf Zeile 886')
        ...
        if abs(cos(th)) > 0:
            ...
            # print('Achtung: Sonderfall theta, Stresses_mixreinf Zeile 893')
        ...
        # print(self.cm_klij)
        # print(self.submodel)
        # if self.sprev_klij.txy > 2:
        #     print(self.sprev_klij.submodel)
        # self.sr1 = self.sr / cos(self.thr - self.th)
        # self.d = self.sr1 * self.e1
        # self.dn_contr = self.d * cos(self.thr - self.th)
        # self.dt_contr = self.d * sin(self.thr - self.th)
    ...
    def sigma_cart_32(self):
        ...
        if self.cm_klij == 4:
            ...
            if self.s1prev < self.fct * ff_fct:
                self.cm_klij = 3
                self.principal()
            elif self.sprev_klij.e3 > -ff_e3:
                self.cm_klij = 3
                self.principal()
            if abs(self.sprev_klij.th) > 80/90*pi/2 or abs(self.sprev_klij.th) < 10/90*pi/2:
                self.cm_klij = 3
                self.principal()
            if self.sprev_klij.submodel == 3 or self.sprev_klij.submodel == 1:
                self.cm_klij = 3
                self.principal()
        ...
        if self.rhox > 0:
            if self.ex > self.ecx:
                self.ssx, self.ssxk = self.ssr(self.ex, self.ecx, self.srx, self.rhox, self.rhopx, self.dx, self.fsyx, self.fsux, self.Esx,
        ...
        if self.rhoy > 0:
            if self.ey > self.ecy:
                self.ssy, self.ssyk = self.ssr(self.ey, self.ecy, self.sry, self.rhoy, self.rhopy, self.dy, self.fsyy, self.fsuy, self.Esy,
        ...
        if self.rhopx > 0:
            if self.ex > self.ecx:
                self.spx, self.spxk = self.spr(self.ex, self.ecx, self.rhopx, self.srx, self.dpx, self.fpux, self.Epx, self.tbp0)
            else:
                self.spx = self.ex*self.Epx
                self.spxk = self.spx
        else:
        ...
        if self.rhopy > 0:
            if self.ey > self.ecy:
                self.spy, self.spyk = self.spr(self.ey, self.ecy, self.rhopy, self.sry, self.dpy, self.fpuy, self.Epy, self.tbp0)
            else:
                self.spy = self.ey*self.Epy
                self.spyk = self.spy
        else:
    ...
    def sigma_cart_33(self):
        ...
        # 2 Steel Contribution
        if self.rhox > 0:
            self.ssx, self.ssxk = self.ssr(self.ex, self.ecx, self.srx0*lbd, self.rhox, self.rhopx, self.dx, self.fsyx, self.fsux, self.Esx,
                                    self.Eshx, self.tb0, self.tb1, self.Ec)
        else:
            self.ssx = 0
            self.ssxk = 0
        if self.rhoy > 0:
            self.ssy, self.ssyk = self.ssr(self.ey, self.ecy, self.sry0*lbd, self.rhoy, self.rhopy, self.dy, self.fsyy, self.fsuy, self.Esy,
                                    self.Eshy, self.tb0, self.tb1, self.Ec)
        else:
            self.ssy = 0
            self.ssyk = 0
        ...
        # 3 CFRP Contribution
        if self.rhopx > 0:
            self.spx, self.spxk = self.spr(self.ex, self.ecx, self.rhopx, self.srx0*lbd, self.dpx, self.fpux, self.Epx, self.tbp0)
        else:
            self.spx = 0
            self.spxk = 0
        if self.rhopy > 0:
            self.spy, self.spyk = self.spr(self.ey, self.ecy, self.rhopy, self.sry0*lbd, self.dpy, self.fpuy, self.Epy, self.tbp0)
        else:
            self.spy = 0
            self.spyk = 0
