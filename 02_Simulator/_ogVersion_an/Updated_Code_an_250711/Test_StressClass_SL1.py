...
# Test Stress Class
# -------------------------------------------------------------------------------------------------------------------- #
# do_itkin funktioniert für start CMM-f bei fct*0.2!
tb0 = 5.2
tb1 = 2.6
tbp0 = 6
tbp1 = 5.9
...
target_stress = True
if target_stress:
    txy_model = np.linspace(.05,2,steps)
    ...
    gxy_model = np.linspace(1,0,steps)/1000
    ex_model = np.linspace(1,5,steps)/1000
    ey_model = np.linspace(0,0,steps)/1000
    sx_model = np.zeros_like(gxy_model)
    sy_model = np.zeros_like(gxy_model)
    txy_model = np.zeros_like(gxy_model)
...
sig_prev = stress(0,1, MAT, GEOM, 1, 1, 1)
[a,sigma_prev,b] = Ableitungsfunktion(sig_prev, np.array([sx_model[0]/Ec, sy_model[0]/Ec, txy_model[0]/Gc, 0.001, 0.001]), 3)
start = time.time()
...
for i in range(len(ex_model)):
    ...
    while contit_i:
        itcount += 1
        sig_prev.thr = 40*pi/180
        sig = stress(sig_prev, 3, MAT, GEOM, 1, 1, 1)
...
# 3 Plot
#-----------------------------------------------------------------------------------------------------------------------
txy_test = [1.27,1.23,1.24,1.24,1.46,1.68,1.79,1.86,1.87]
ex_test = [0.02,0.08,0.36,0.81,1.06,1.41,1.64,1.79,1.86]
ey_test = [0.04,0.10,0.52,1.40,2.06,3.44,5.00,6.27,6.70]
gxy_test = [0.13,0.22,0.94,2.24,3.18,4.76,6.32,7.48,7.88]
dn_test = [0.01,0.02,0.11,0.32,0.43,0.67,0.86,0.99,1.03]
dt_test = [0.00,0.00,0.00,0.02,0.04,0.12,0.21,0.29,0.32]
ssx_test = [1.2,8.8,86.5,173.6,216.6,259.8,288.5,306,308]
ssy_test = [2,13.7,152.2,364,378.6,448.8,497.5,514.6,520
            ]
...
axs[0, 0].plot(ex_model, sx2, 'r', linestyle='dashed')
...
axs[0, 1].plot(ey_model, sx2, 'r', linestyle='dashed')
...
axs[0, 2].plot(gxy_model, sx2, 'r', linestyle='dashed')
...
axs2[0, 0].plot(ex_model, sy2, 'r', linestyle='dashed')
...
axs2[0, 1].plot(ey_model, sy2, 'r', linestyle='dashed')
...
axs2[0, 2].plot(gxy_model, sy2, 'r', linestyle='dashed')
...
axs3[0, 0].plot(ex_model, txy2, 'r', linestyle='dashed')
...
axs3[0, 1].plot(ey_model, txy2, 'r', linestyle='dashed')
...
axs3[0, 2].plot(gxy_model, txy2, 'r', linestyle='dashed')
axs3[0, 2].scatter(gxy_test,txy_test,s=5,color='k')
...
axs4[1, 1].scatter(gxy_test,dn_test,s=5,color='r')
axs4[1, 1].scatter(gxy_test,dt_test,s=5,color='m')
...
axs4[2, 2].scatter(gxy_test,ssx_test,s=5,color='b')
axs4[2, 2].scatter(gxy_test,ssy_test,s=5,color='c')
