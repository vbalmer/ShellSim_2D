...
class stress():
    ...
    def ssr(self, e, ec, ecs, srm, rho, rhop, d, fsy, fsu, Es, Esh, tb0, tb1, Ec):
        ...
        # Flag
        # 4 No values below ssr0
        # 4.1 Strain where Seelhofer and fct intersect
        # if e == self.ey:
        #     dp = self.dpy
        #     Ep = self.Epy
        # else:
        ...
        #     Ep = self.Epx
        #
        # er0 = (self.fct*(1-rho-rhop)-self.tb0*srm/d*rho-self.tbp0*srm/dp*rhop)/(Es*rho+Ep*rhop)+ecs
        # ssr0 = er0*Es+self.tb0*srm/d
        # if ssr < ssr0 and e > self.fct/self.Ec:
        #     ssr = ssr0
        #     ssk = ssr0
    ...
    def spr(self, e, ec, ecs, rhop, srm, dp, fpu, Ep, tb):
        ...
        #     rho = self.rhoy
        #
        # er0 = (self.fct * (1 - rho - rhop) - self.tb0 * srm / d * rho - self.tbp0 * srm / dp * rhop) / (
        #             Es * rho + Ep * rhop) + ecs
        # spr0 = er0 * Ep + self.tbp0 * srm / dp
        # if spr < spr0 and e-ecs > self.fct / self.Ec:
        #     spr = spr0
        #     spk = spr0
