...
new_data_inp = new_data_inp[:,0:11]                         # only for RC
...
# Save split data
save_path = os.path.join(path, '04_Training/new_data')
...
# Plot to check
histogram(X_train[:,0:8], y_train[:,0:8], amt_data_points, nbins, 'eps', path_plots)
histogram(X_train[:,0:8], y_train[:,0:8], amt_data_points, nbins, 'sig', path_plots)
histogram(X_train[:,8:], y_train[:,0:8], amt_data_points, nbins, 't2', path_plots)
histogram(X_train[:,8:], y_train[:,8:].reshape((-1,8,8)), amt_data_points, nbins, 'De', path_plots)
...
# Plot to check
histogram(X_train_t[:,0:8], y_train_t[:,0:8], amt_data_points, nbins, 'eps', path_plots)
histogram(X_train_t[:,0:8], y_train_t[:,0:8], amt_data_points, nbins, 'sig', path_plots)
histogram(X_train_t[:,8:], y_train_t[:,0:8], amt_data_points, nbins, 't2', path_plots)
histogram(X_train[:,8:], y_train_t[:,8:].reshape((-1,8,8)), amt_data_points, nbins, 'De', path_plots)
...
loaders, mat_torch = data_to_torch(X_train_t, y_train_t, X_eval_t, y_eval_t, 
                        X_test_t, y_test_t, 
                        save_path, SOBOLEV, batch_size, tag = (PURE_D or INV))
...
inp = {
    ...
    'input_size': 9,                                # 8 eps + t (thickness) = 9 . For DeepONet use 8 as input size!
    'out_size': 8,                                  # 8 sig, or: 38 D-matrix, or 1: energy
    'hidden_layers': str([64]*10),
    'batch_size': batch_size,                       # Defined in Step ##3 above, DO NOT CHANGE HERE
    'num_epochs': 30000,
    'activation': 'ELU',
    'learning_rate': 0.005,
    ...
    'w_s': str([0,1]),                                   # weights [w1, w2] for 1st, 2nd order loss with sobolev (neglected if sobolev = False), or 'max'
