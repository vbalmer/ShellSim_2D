...
De = np.load(os.path.join(os.getcwd(),"04_Training\\data\\data_20241022_1853_case4\\new_data_De.pkl"), allow_pickle=True)
eps = np.load(os.path.join(os.getcwd(), "04_Training\\data\\data_20241022_1853_case4\\new_data_eps.pkl"), allow_pickle=True)
sig = np.load(os.path.join(os.getcwd(), "04_Training\\data\\data_20241022_1853_case4\\new_data_sig.pkl"), allow_pickle=True)
t = np.load(os.path.join(os.getcwd(), "04_Training\\data\\data_20241022_1853_case4\\new_data_t.pkl"), allow_pickle=True)
...
for epoch in range(MAXEPOCHS):
    model.train()
    optimizer.zero_grad()
    X_torch = torch.tensor(normalizedX, dtype=torch.float32).to(device)
    sig_torch = torch.tensor(normalizedSig, dtype=torch.float32).to(device)
...
model.eval()
normalizedPred = model(X_torch).cpu().detach().numpy()
predictions = normalizedPred * (np.max(sig, axis=0) - np.min(sig, axis=0)) + np.min(sig, axis=0)

error = abs(normalizedPred - normalizedSig)
